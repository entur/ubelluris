...
class TransportModeToLocalScheduledStopPointMapperTest {
    ...
    private fun runMapper(
        ...
        resourceFileNames.forEachIndexed { index, fileName ->
            val bytes = requireNotNull(javaClass.getResourceAsStream("/timetable/$fileName")) {
    ...
    fun shouldMapTramStopsToLocalIds() {
        ...
        assertThat(result["1:101"]).containsExactly(TransportMode.TRAM)
        assertThat(result["1:102"]).containsExactly(TransportMode.TRAM)
    }
    ...
    fun shouldMapWaterStopsToLocalIds() {
        ...
        assertThat(result["1:201"]).containsExactly(TransportMode.WATER)
    }
    ...
    fun shouldIncludeOnlyConfiguredModes() {
        ...
        assertThat(result["1:301"]).containsExactly(TransportMode.BUS)
        assertThat(result).doesNotContainKey("1:101")
        assertThat(result).doesNotContainKey("1:201")
    }

    @Test
    fun shouldAccumulateResultsAcrossMultipleFiles() {
        val result = runMapper(
